...
reg [MATRIXSIZE_W-1:0] block_count = 0;
    ...
    if (rst) begin
        ...
        if (col == N2-1) begin
            ...
            if (phase == M3dN2-1) begin
                ...
                if (row == M2-P_B) begin
                    row <= 0;
                    block_count <= (block_count == BLOCKS-1) ? 0 : block_count + 1;
                end
            end
        end
...
always @(posedge clk) begin
    if (rst) begin
        ...
        if (valid_B) begin
            ...
            activate_B_rr[0] <= (row == M2-1) && (phase == M3dN2-1) && (block_count == BLOCKS-1) ? 0 : activate_B_rr[N2-1];
